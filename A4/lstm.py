...
X, y = create_sequences(data, feature_cols, target_col, window_size)
...
model = TransformerTimeSeries(input_size, d_model=64, nhead=4, num_layers=3, dropout=0.1).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
...
for epoch in tqdm(range(50)):
    ...
    for X_batch, y_batch in train_loader:
        optimizer.zero_grad()
        loss = criterion(model(X_batch.to(model.decoder.weight.device)), y_batch.to(model.decoder.weight.device))
        loss.backward()
        optimizer.step()
...
plt.savefig("transformer_loss_curve1.png")
plt.show()
